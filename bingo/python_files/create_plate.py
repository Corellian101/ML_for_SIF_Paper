...
def create_model(b=12.0):
    ...
    s1.rectangle(point1=(-b, -b), point2=(b, b))
    ...
    a = mdb.models['Model-5'].rootAssembly
    e1 = a.instances['Part-1-1'].edges
    edges1 = e1.getSequenceFromMask(mask=('[#2200011 #12 ]', ), )
...
def create_global(x_cut=7,y_cut=2,seed=0.6):
    import section
    import regionToolset
    import displayGroupMdbToolset as dgm
    import part
    ...
    s1.rectangle(point1=(-x_cut, -y_cut), point2=(x_cut, y_cut))
    p = mdb.models['Model-5_global'].parts['Part-1']
...
def create_local(b=12,x_cut=7,y_cut=2,seed=0.2):
    import section
    import regionToolset
    import displayGroupMdbToolset as dgm
    import part
    ...
    p = mdb.models['Model-5_local'].parts['Part-1']
    p.projectReferencesOntoSketch(sketch=s, filter=COPLANAR_EDGES)
    s.rectangle(point1=(-b, -b), point2=(b, b))
    s.rectangle(point1=(-x_cut, -y_cut), point2=(x_cut, y_cut))
    p = mdb.models['Model-5_local'].parts['Part-1']
    f, e1 = p.faces, p.edges
    p.CutExtrude(sketchPlane=f[10], sketchUpEdge=e1[46], sketchPlaneSide=SIDE1, 
    ...
    nodes = []
    for i in mdb.models['Model-5_local'].parts['Part-1'].sets['cut_local'].nodes:
...
def mesh_model():
    ...
    mdb.Job(name='full_model', model='Model-5', description='', type=ANALYSIS, 
        ...
        scratch='', resultsFormat=ODB, multiprocessingMode=DEFAULT, numCpus=1, 
        numGPUs=0)
    mdb.jobs['full_model'].writeInput(consistencyChecking=OFF)
